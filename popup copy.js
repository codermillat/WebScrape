...
(function() {
  ...
  // Gemini API client (internal use only; API key hardcoded per request)
  const GeminiClient = (() => {
    ...
    async function callGemini(model, prompt) {
      ...
      const res = await fetchWithTimeout(endpoint(model), {
        method: 'POST',
  ...
  // DigitalOcean AI fallback client (Llama 3.3 70B Instruct)
  const DOClient = (() => {
    const API_KEY = 'DO_API_KEY';
    ...
    try {
      ...
      try {
        response = await Promise.race([
          chrome.tabs.sendMessage(tab.id, { action: 'extractText', includeHidden: true, autoScroll: true }),
  ...
  function updateDisplayedContent(extractedTables = []) {
    ...
    let displayContent = '';
    let filename = generateTitleFilename();
    ...
    switch (format) {
      ...
      case 'json':
        ...
        // Display JSON but save as .txt per requirements
        break;
      default:
        displayContent = processedData.processedText;
    }
   ...
   */
  function generateTitleFilename() {
